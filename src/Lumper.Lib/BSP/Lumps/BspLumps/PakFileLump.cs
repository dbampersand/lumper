...
public partial class PakfileLump(BspFile parent) : ManagedLump<BspLumpType>(parent), IFileBackedLump
{
    ...
    // Matches cubemap names which are formatted as cX_cY_cZ.vtf or cubemapdefault.vtf, including .hdr.vtf versions, where X Y Z are the cubemap's origin
    // https://github.com/ValveSoftware/source-sdk-2013/blob/master/mp/src/utils/vbsp/cubemap.cpp
    [GeneratedRegex("materials/maps/(.*)/((c-?d+_-?d+_-?d+(.hdr)*.vtf)|(cubemapdefault(.hdr)*.vtf))")]
    public static partial Regex CubemapRegex();
    /// <summary>
